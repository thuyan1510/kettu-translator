(function (metro, toasts, alerts, assets, plugin) {
    ...
    async function translateText(text, target) {
        ...
        try {
            ...
            if (typeof res.translated === 'string')
    ...
    function patchMessageMenu() {
        const unpatch = patchAll("message", (ctx) => {
            if (!ctx.message)
                return;
            const translateItem = {
                label: "Dịch",
    ...
    function patchAll(...args) {
        const unpatchFunc = metro.getByProps(...args);
        return unpatchFunc;
    }
    // --- Nút bật/tắt Auto Translate trên thanh chat (CẬP NHẬT) ---
    function addChatBarButton() {
        const ChatInput = metro.getByProps("ChatInput")?.ChatInput;
        if (!ChatInput)
            return;
...
})(metro, toasts, alerts, assets, plugin);
